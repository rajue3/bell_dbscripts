...
@result int OUTPUT ,
@OPERATION AS VARCHAR(30) = '' , 
@ISFORDIRECTSALES AS VARCHAR(3) = 'NO',  
...
END
IF (@OPERATION='UPDATE' OR @OPERATION='')
 BEGIN
	PRINT 'UPDATING...'
	 IF NOT EXISTS(SELECT ID FROM Bell_Cust_Master WHERE LINE=@LINE AND AREA=@AREA AND  SHOPNAME=@SHOP AND STATUS='ACTIVE')  
	 BEGIN  
	  INSERT INTO Bell_Cust_Master (CUSTID,LINE,AREA,SHOPNAME,SALESMAN,CUSTOMERNAME,MOBILE,USERNAME,[STATUS],ISFORDIRECTSALES,GROUPNAME,CATEGORY) VALUES(@CUSTID,@LINE,@AREA,@SHOP,@SALESMAN,@CUSTOMERNAME,@MOBILE,@USERNAME,'Active',@ISFORDIRECTSALES,@GROUPNAME,@CATEGORY)  
	 END  
	  ELSE  
	  BEGIN  
		  UPDATE Bell_Cust_Master SET LINE=@LINE,AREA=@AREA,ShopName=@SHOP,SALESMAN=@SALESMAN,CustomerName=@CUSTOMERNAME,MOBILE=@MOBILE,  
		  USERNAME=@USERNAME,ACTIONDATE=GETDATE(),STATUS='Active',ISFORDIRECTSALES=@ISFORDIRECTSALES,GROUPNAME=@GROUPNAME,CATEGORY=@CATEGORY 
		  WHERE LINE=@LINE AND AREA=@AREA AND  SHOPNAME=@SHOP
	  END	   
 END  
	SET @result = 1  

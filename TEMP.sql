...
BEGIN        
 ...
 --ITEMCODE,ITEMNAME,AREA,QTY    
   ...
   END        
END        
else IF lower(@TYPE) = 'byweeklycategoryname'        
BEGIN        
        
 CREATE TABLE dbo.#TEMP_WEEKLY_BY_CATEGORY (AREA VARCHAR(50),ITEMCODE INT,ITEMNAME VARCHAR(50),QTY INT)        
  ...
  begin        
   ...
   set @script = N' Insert into #TEMP_WEEKLY_BY_CATEGORY        
  select DISTINCT AREA,ITEMCODE,ITEMNAME,ISNULL(SUM(PACKETS),0) QTY from bhavani_ER_Bills        
  WHERE     
  (BILLDATE BETWEEN CONVERT(nvarchar(10),'''+ Cast(@BILLDATE1 as varchar(15)) +''',101) AND CONVERT(nvarchar(10),'''+ cast(@BILLDATE2 as varchar(15)) + ''',101)) ' + @query2   + @queryItemNames  
  + ' AND ISNULL(ITEMCODE,0) > 0 GROUP BY AREA,ITEMCODE,ITEMNAME '        
  ...
  CREATE TABLE dbo.#TEMP_HEADER_WEEKLY (HEADER_NAME VARCHAR(50))        
        
   if (select count(1) from #TEMP_WEEKLY_BY_CATEGORY) > 0         
   BEGIN        
    --PRINT 'RECORDS EXISTS'        
    INSERT INTO #TEMP_HEADER_WEEKLY SELECT DISTINCT AREA FROM #TEMP_WEEKLY_BY_CATEGORY
    --select * from #TEMP_HEADER        
    
    ----*FOR PIVOTE, FIRST GET DISTINCT COLUMN NAMES AND THEN USE PIVOTE        
    select @cols = STUFF((SELECT ',' + '['+ QUOTENAME(HEADER_NAME) + ']' from #TEMP_HEADER_WEEKLY group by HEADER_NAME     
 FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)') ,1,1,'')     
          
   set @query = N'SELECT * from (select * from #TEMP_WEEKLY_BY_CATEGORY ) x           
       pivot (sum(QTY) for AREA in (' + REPLACE(REPLACE(@cols,'[[','['),']]',']') + N') ) P ORDER BY ITEMCODE'    
   PRINT @query          
   exec sp_executesql @query;          
   END        

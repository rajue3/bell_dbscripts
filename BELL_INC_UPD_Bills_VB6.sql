...
BEGIN    
 ...
 BEGIN    
		insert into bhavani_ER_Bills (ITEMCODE,ITEMNAME,RATE,PACKETS,QTY,AMOUNT,BILLNUMBER,BILLDATE,AREA,SHOPNAME,USERNAME,PRATE,AREA_LINE)     
		values(@ITEMCODE,@ITEMNAME,@PRICE,@PACKETS,@QTY,@AMOUNT,@BILLNUMBER,@BILLDATE,@AREA,@SHOP,@USERNAME,@PRATE,ISNULL(@AREA_LINE,@AREA))    
		
		if (select count(1) from Bell_Cust_Master  where line = @area  and IsForDirectSales='YES')  > 0 
		begin
  ...
  BEGIN    
		...
		,AREA_LINE=ISNULL(@AREA_LINE,@AREA)  WHERE    
        ITEMCODE=@ITEMCODE AND ITEMNAME=@ITEMNAME AND AREA=@AREA AND SHOPNAME=@SHOP AND  

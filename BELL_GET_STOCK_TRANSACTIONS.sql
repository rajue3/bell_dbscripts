...
-- BELL_GET_STOCK_TRANSACTIONS '[SWEETS],[BISCUITES]','ALL','2024-12-01','2024-12-30'  
-- BELL_GET_STOCK_TRANSACTIONS '[Raw Materials]','IN','2024-12-01','2024-12-30'  
-- BELL_GET_STOCK_TRANSACTIONS 'SWEETS','OUT','2024-12-01','2024-12-30'  
-- BELL_GET_STOCK_TRANSACTIONS 'ALL','ALL','2024-12-01','2024-12-30'  
...
--(CASE LOWER(A.TRANS_TYPE) WHEN 'C' THEN 'IN' ELSE 'OUT' END) AS [IN/OUT]    
  ...
  BEGIN  
    ...
    FROM BELL_LS LS  
    INNER JOIN BELL_ITEMMASTER B ON B.ITEMCODE=LS.ITEMCODE AND B.ITEMNAME=LS.ITEMNAME   
    WHERE (LS.BILLDATE BETWEEN '''+CONVERT(nvarchar(10),@FROMDATE,101)+''' AND '''+CONVERT(nvarchar(10),dateadd(d,1,@TODATE),101)+''') ' + @query2         
  END  

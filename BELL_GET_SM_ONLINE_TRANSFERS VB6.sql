...
--BELL_GET_SM_ONLINE_TRANSFERS  '01-Nov-2025','CHENNURU','RAMESH.P','','01-Nov-2025',0,0,'PENDING_COMM_DATES'                
--select distinct paiddate from BELL_DAILYCASHFLOW where Transtype='IN' and purpose='CHENNURU' and paidtoperson='RAMESH.P' and paiddate<'01-Nov-2025'            
...
--   due_date is not null and category='CHENNURU' and paidtoperson='RAMESH.P' and paiddate<'01-Nov-2025' )      
...
-- BELL_GET_SM_ONLINE_TRANSFERS  '28-Mar-2025','','','','06-Nov-2025',0,0,'SALES_COMM_PAID'                
-- BELL_GET_SM_ONLINE_TRANSFERS  '01-Apr-2025','','','','06-Nov-2025',0,0,'LINE_SALES_COMM'                  
...
--  set @STRSQLWHERE = @STRSQLWHERE + ' AND Billdate>=''' + @BILLDATE + ''''                
 ...
 --from bell_dailycashflow a  where transtype=''OUT'' and (a.category like ''%COMMISSION%'' or purpose like ''%COMMISSION%'')   '   
  ...
  with CTE as (
  select distinct a.* 
  from bell_dailycashflow a
  where a.transtype='IN' and a.category='LINE_SALES' and 
  purpose in (select distinct line from bell_lines where status='Active' and line is not null) 
  --a.purpose not in ('MOSES SIR','DUE AMOUNT RECEIVED','DUE AMOUNT PAY','ITEMS SALE ') and a.purpose not like '%- Due'
  and a.paiddate>=@BILLDATE and a.paiddate <=@PAYMODE and purpose not in ('NEZAR','BHAVANI' ,'GATE','BAZAR')
 and (a.due_date is null OR a.due_date='1900-01-01')
 )
	select a.*,COMMISION_DATE=(select top 1 b.paiddate from bell_dailycashflow b where A.paiddate=B.due_date and b.transtype='OUT'
	and (b.category like '%COMMISSION%' or b.purpose like '%COMMISSION%')  and b.paiddate>=@BILLDATE and b.paiddate <=@PAYMODE  and b.due_date is not null and b.due_date<>'1900-01-01' and B.PAIDTOPERSON=A.PAIDTOPERSON AND a.PURPOSE=b.CATEGORY ) 
		 ,COMMISION_AMT=(select top 1 b.Amount from bell_dailycashflow b where A.paiddate=B.due_date and b.transtype='OUT'
	and (b.category like '%COMMISSION%' or b.purpose like '%COMMISSION%')  and b.paiddate>=@BILLDATE and b.paiddate <=@PAYMODE  and b.due_date is not null and b.due_date<>'1900-01-01' and B.PAIDTOPERSON=A.PAIDTOPERSON AND a.PURPOSE=b.CATEGORY ) 

	 from  CTE a where a.purpose=(case when @LINE='' then a.purpose else @LINE end)    
    and a.paidtoperson=(case when @SALESMAN='' then a.paidtoperson else @SALESMAN end)             
    order by a.paiddate desc             
 end                
...
set @SCRIPT2 =' ) as Advance  from BELL_CATEGORY_MASTER a where category=''Salary'' and fieldtype=''out'' and isnull(person,'''') <> ''''  '                  
 ...
 UNION            
  ...
  Billdate is null and due_cashid is null            
  and line=(case when @LINE='' then line else @LINE end)            
  and salesman=(case when @SALESMAN='' then salesman else @SALESMAN end)     

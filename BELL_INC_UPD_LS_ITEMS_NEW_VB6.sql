...
BEGIN      
 ...
 if @USERNAME = 'ORDERS' 
  BEGIN
		-- ADDED ON 11-07-2026 
		--** SHOULD NOT ADD THIS. DELETE FROM BELL_LS_ORDERS  WHERE USERNAME = 'ORDERS'  AND AREA=@AREA AND CONVERT(varchar(10),BILLDATE,101) = @BILLDATE
		...
		ELSE
		BEGIN			
			if (@T_B) = 0  -- THIS IS TO DELETE AN EXISTING RECORD WHITH EMPTY PACKS
			BEGIN
					DELETE FROM BELL_LS_ORDERS WHERE ITEMCODE=@ITEMCODE AND ITEMNAME=@ITEMNAME AND AREA=@AREA AND BILLDATE = @BILLDATE                
			END
			ELSE
			BEGIN

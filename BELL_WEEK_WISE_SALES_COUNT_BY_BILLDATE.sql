...
-- BELL_WEEK_WISE_SALES_COUNT_BY_BILLDATE 'monthlyitemwisesale','all','all','all','2025-07-01','2025-11-30'    
 ...
 -- BELL_WEEK_WISE_SALES_COUNT_BY_BILLDATE 'bazar-dailyitemwisesales','BHAVANI','all','all','2026-07-26','2026-08-10'    
 -- BELL_WEEK_WISE_SALES_COUNT_BY_BILLDATE'bazar-billwisesales' ,'BHAVANI','all','all','2026-07-26','2026-08-10'    
...
--TODO: FOR bazar shops.
 if @TYPE='bazar-dailyitemwisesales'    
  BEGIN  
    select ISNULL(ITEMCODE,0) ITEMCODE,ITEMNAME,SUM(PACKETS) AS QTY,RATE,
    CAST(ISNULL(SUM(AMOUNT)/100.0,0) AS DECIMAL(12,2)) AMOUNT,
    CONVERT(nvarchar(10),BILLDATE,101) BILLDATE
    from Bazar_Mobile_Bills 
     WHERE ISNULL(ITEMCODE,0) > 0  AND LINE=@AREA and AMOUNT > 0 AND
     ...
     GROUP BY BILLDATE,ITEMNAME,LINE,ITEMCODE,RATE
     order by ITEMCODE
  END     
if @TYPE='bazar-billwisesales'    
  BEGIN  
    select BILLNUMBER,
    CAST(ISNULL(SUM(AMOUNT)/100.0,0) AS DECIMAL(12,2)) AMOUNT,
    CONVERT(nvarchar(10),BILLDATE,101) BILLDATE
    from Bazar_Mobile_Bills WHERE LINE=@AREA and AMOUNT > 0 AND
     (BILLDATE BETWEEN CONVERT(nvarchar(10), Cast(@BILLDATE1 as varchar(15)),101) AND CONVERT(nvarchar(10), cast(@BILLDATE2 as varchar(15)),101)) 
     GROUP BY BILLDATE,BILLNUMBER order by BILLDATE,BILLNUMBER
  END     
if @TYPE='bazar-weeklyitemwisesales'    
  BEGIN  
    select CONVERT(varchar(20),BILLDATE)  AS BILLDATE,          
     ISNULL(ITEMCODE,0) ITEMCODE,ITEMNAME,CAST(ISNULL(SUM(AMOUNT)/100.0,0) AS DECIMAL(12,2)) AMOUNT  from Bazar_Mobile_Bills 
     WHERE ISNULL(ITEMCODE,0) > 0  AND LINE=@AREA
     GROUP BY BILLDATE,ITEMNAME,LINE,ITEMCODE 
     order by ITEMCODE
     ...
     ISNULL(ITEMCODE,0),ITEMNAME,CAST(ISNULL(SUM(AMOUNT)/100.0,0) AS DECIMAL(12,2)) AMOUNT  from Bazar_Mobile_Bills 
     WHERE ISNULL(ITEMCODE,0) > 0 GROUP BY BILLDATE,ITEMNAME,AREA,ITEMCODE order by ITEMCODE

      set @query = N' Insert into #TEMP_REPORT_ITEMWISE_BAZAR          
     select CONVERT(varchar(20),DATEADD(wk, DATEDIFF(wk, 6,''1/1/'' + CAST(DATEPART(YY, BILLDATE) AS CHAR(4))) + (DatePart(week, BILLDATE)-1), 6),6) AS StartOfWeek,          
     ...
     + ' AND ISNULL(ITEMCODE,0) > 0 GROUP BY BILLDATE,ITEMNAME,AREA,ITEMCODE order by ITEMCODE  '    
      ...
      set @query = N' INSERT INTO Bell_REPORT_HEADER_BILLDATE SELECT DISTINCT CONVERT(varchar,BILLDATE,6) AS BILLDATE FROM #TEMP_REPORT_ITEMWISE_BAZAR '           
      EXEC sp_executesql @query              
      ...
      set @query = 'SELECT * from (select * from #TEMP_REPORT_ITEMWISE_BAZAR  '            
...
if lower(@ITEMNAME) <> 'all'    
 ...
 BEGIN  
 print 'monthlyitemwisesales report from second loop'
  set @query = N' Insert into #TEMP_REPORT_ITEMWISE_6          
  select convert(varchar(20), + ''01 '' +LEFT(DATENAME(MONTH,BILLDATE), 3) + '' '' + CAST(RIGHT(YEAR(BILLDATE),2) AS CHAR(2)) ) MonthYear,          
  ISNULL(ITEMCODE,0),ITEMNAME,CAST(ISNULL(SUM(AMOUNT)/100.0,0) AS DECIMAL(12,2)) AMOUNT  from bhavani_ER_Bills          
  WHERE   

...
SELECT BillDate, ROW_NUMBER() OVER (ORDER BY BillDate DESC) AS rn
 ...
 BELL_GET_PREVIOUS4_ORDERS_NEW 'BAZAR','2026-May-06' ,'BILLS2'

 BELL_GET_PREVIOUS4_ORDERS_NEW 'ITEM SALE IN FACTORY','2026-May-06' ,'BILLDATES'
 BELL_GET_PREVIOUS4_ORDERS_NEW 'bhavani','2026-May-06' ,'BILLDATES2'
 
 BELL_GET_PREVIOUS4_ORDERS_NEW 'JANGAON','2026-May-06' ,'BILLDATES'
 BELL_GET_PREVIOUS4_ORDERS_NEW 'JANGAON','2026-May-06' ,'BILLDATES2' 
 BELL_GET_PREVIOUS4_ORDERS_NEW 'KORUTLA','2026-Apr-30' ,'BILLS2' 

 BELL_GET_PREVIOUS4_ORDERS_NEW 'JANGAON','2026-Apr-30'  -- using in VB6
 BELL_GET_PREVIOUS4_ORDERS_NEW 'JANGAON','2026-Apr-30' ,'BILLS'
 BELL_GET_PREVIOUS4_ORDERS_NEW 'JANGAON','2026-Apr-30' ,'BILLS_LS'
 BELL_GET_PREVIOUS4_ORDERS_NEW 'JANGAON','2026-Apr-30','BILLS'  -- using in Mobile App
 BELL_GET_PREVIOUS4_ORDERS_NEW 'JANGAON','2026-Apr-30','BILLDATES'
BELL_GET_PREVIOUS4_ORDERS_NEW 'JANGAON','2026-Apr-30','WITH_BILLDATE'
BELL_GET_PREVIOUS4_ORDERS_NEW 'JAMMIKUNTA','2026-Apr-30','WITH_BILLDATE'

SELECT *	FROM Bhavani_ER_Bills WITH (NOLOCK)
WHERE Area='JANGAON'  AND SHOPNAME LIKE 'HARI HARA%' AND BillDate in ('2026-Apr-07' ,'2026-Apr-14','2026-Apr-21','2026-Apr-28'   )
...
END
ELSE IF @REPORTTYPE='BILLS2'  -- using for Direct Sales Lines. Bazar, Gate...
BEGIN
		--WITH LastDates AS (
		--	SELECT BillDate,
		--		   ROW_NUMBER() OVER (ORDER BY BillDate DESC) AS rn
		...
		--)
		--, Filtered AS (
		--	SELECT ItemCode,ItemName,BillDate, PACKETS,AREA	FROM Bhavani_ER_Bills WITH (NOLOCK)	WHERE Area=@AREA
		--	  AND BillDate IN (SELECT BillDate FROM LastDates WHERE rn <= 4)
		--)  		
		--SELECT ItemCode,ItemName, AREA,
		...
		--	FROM Filtered f LEFT JOIN LastDates ld ON f.BillDate = ld.BillDate						
		--	GROUP BY ItemCode,ItemName,AREA  ORDER BY ItemCode
		;WITH LastDates AS (
		...
		JOIN LastDates ld ON f.BillDate = ld.BillDate AND f.Area = @AREA
		GROUP BY f.ItemCode, f.ItemName, f.Area
		ORDER BY f.ItemCode;
...
ELSE if @REPORTTYPE='BILLDATES2'  -- using for Lines with Direct Sales from office. ex: Bazar, Gate...
begin
		--SELECT DISTINCT TOP 4 BILLDATE,AREA FROM BELL_LS WHERE AREA=@AREA and BillDate<@BILLDATE ORDER BY BILLDATE DESC
		;WITH LastDates AS (
			SELECT DISTINCT BillDate FROM Bhavani_ER_Bills with (nolock) WHERE Area = @AREA   AND BillDate < @BILLDATE

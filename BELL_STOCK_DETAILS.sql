/* SELECT ITEMCODE,ITEMNAME,ISNULL(PRATE,0) AS PRATE,ISNULL(RATE1,0) AS RATE, ISNULL(MRP,0) MRP, ISNULL(CATEGORY,'') CATEGORY,Manufacture,     
 ...
 SELECT * FROM BELL_ITEMMASTER WHERE STOCK IS NULL
 UPDATE BELL_ITEMMASTER  SET STOCK = 1000 
...
BEGIN     
 ...
 set @script = N' SELECT ITEMCODE,ITEMNAME,ISNULL(PRATE,0) AS PRATE,ISNULL(RATE1,0) AS RATE,ISNULL(RATE1,0) AS RATE1,ISNULL(RATE2,0) AS RATE2, ISNULL(MRP,0) MRP, ISNULL(CATEGORY,'''') CATEGORY, ISNULL(Manufacture,'''') Manufacture,    
 ISNULL(MinOrderAlert,0) MinOrderAlert, isnull(TOTALITEMSINPACK,0) TOTALITEMSINPACK,
 CAST(ISNULL(MinOrderAlert,0) AS NVARCHAR(5)) + '' '' + PACKINGTYPE as [Description],    
 ISNULL(CAST((STOCK/TOTALITEMSINPACK) AS NVARCHAR(10) ) + '''' + CAST(LEFT(PACKINGTYPE, 1) AS CHAR(1)) + '' ('' + CAST(STOCK%TOTALITEMSINPACK AS NVARCHAR(10))+ ''P) '','''') AS QTY,     
 PACKINGTYPE,ISNULL(STOCK/TOTALITEMSINPACK,0) AS CARTONS,ISNULL(STOCK%TOTALITEMSINPACK,0) AS PACKETS,ITEM_SEQ,  
 ISNULL(STOCK,0) AS STOCK FROM BELL_ITEMMASTER where Status=''Active'' ' + @query2  + ' ORDER BY ITEM_SEQ'
...
BEGIN    
 ...
 CAST(ISNULL(MinOrderAlert,0) AS NVARCHAR(5)) + '' '' + PACKINGTYPE as [Description],    
 ISNULL(CAST((STOCK/TOTALITEMSINPACK) AS NVARCHAR(10) ) + '''' + CAST(LEFT(PACKINGTYPE, 1) AS CHAR(1)) + '' ('' + CAST(STOCK%TOTALITEMSINPACK AS NVARCHAR(10))+ ''P) '','''') AS QTY,     
 PACKINGTYPE,STOCK/TOTALITEMSINPACK AS CARTONS,STOCK%TOTALITEMSINPACK AS PACKETS,ITEM_SEQ,
 ISNULL(STOCK,0) AS STOCK FROM BELL_ITEMMASTER where Status=''Active''     
 AND (STOCK/TOTALITEMSINPACK) <= MINORDERALERT ' + @query2   + ' ORDER BY ITEM_SEQ'
END  
